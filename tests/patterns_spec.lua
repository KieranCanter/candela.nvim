describe("candela.patterns", function()
    ...
    describe("selection", function()
        it("starts empty", function()
            assert.is_true(vim.tbl_isempty(patterns.get_selected()))
        end)

        it("add_to_selected adds regex", function()
            patterns.add("ERROR")
            patterns.add_to_selected("ERROR")
            assert.is_true(patterns.selected["ERROR"])
        end)

        it("remove_from_selected removes regex", function()
            patterns.add("ERROR")
            patterns.add_to_selected("ERROR")
            patterns.remove_from_selected("ERROR")
            assert.is_nil(patterns.selected["ERROR"])
        end)

        it("add_all_to_selected selects all patterns", function()
            patterns.add("ERROR")
            patterns.add("WARN")
            patterns.add_all_to_selected()
            assert.is_true(patterns.selected["ERROR"])
            assert.is_true(patterns.selected["WARN"])
        end)

        it("clear_selected empties selection", function()
            patterns.add("ERROR")
            patterns.add_to_selected("ERROR")
            patterns.clear_selected()
            assert.is_true(vim.tbl_isempty(patterns.selected))
        end)

        it("get_selected returns the selected table", function()
            patterns.add("ERROR")
            patterns.add_to_selected("ERROR")
            local sel = patterns.get_selected()
            assert.is_true(sel["ERROR"])
        end)

        it("deleting a pattern removes it from selected", function()
            patterns.add("ERROR")
            patterns.add_to_selected("ERROR")
            patterns.delete("ERROR")
            assert.is_nil(patterns.selected["ERROR"])
        end)

        it("clearing patterns clears selection", function()
            patterns.add("ERROR")
            patterns.add("WARN")
            patterns.add_to_selected("ERROR")
            patterns.clear()
