describe("candela.lightbox", function()
    ...
    describe("init", function()
        it("sets win_config from system-vsplit", function()
            lightbox.init()
            assert.is_not_nil(lightbox.win_config)
            assert.equals("vsplit", lightbox.open_command)
        end)

        it("is idempotent", function()
            lightbox.init()
            local cfg = lightbox.win_config
            lightbox.init()
            assert.equals(cfg, lightbox.win_config)
        end)
    end)

    describe("init with split direction", function()
        it("parses split-right", function()
            package.loaded["candela.config"].options.lightbox.view = "split-right"
            package.loaded["candela.lightbox"] = nil
            lightbox = require("candela.lightbox")
            lightbox.init()
            assert.equals("right", lightbox.win_config.split)
            assert.is_nil(lightbox.open_command)
        end)
    end)
    ...
    describe("toggle", function()
        it("opens and closes", function()
            ...
            -- Focus the lightbox window so toggle closes it
